				...
				enum omap_dss_rotation_angle *rotation)
{
	...
	case 90:
		//[jonathan.kim@lge.com] S rotation patch from TI 20101219
		//*rotation = OMAP_DSS_ROT_270;
		*rotation = OMAP_DSS_ROT_90;
		//[jonathan.kim@lge.com] E rotation patch from TI 20101219
		break;
	case 180:
...
// jonathan.kim@lge.com INDIA camp patch S
...
// jonathan.kim@lge.com INDIA camp patch E
{
	int r;
...
/* LGE_CHANGE_S [raejoo.ha@lge.com] 2010-12-08 S3D Extension*/
#if 0
...
#endif
/* LGE_CHANGE_E [raejoo.ha@lge.com] 2010-12-08 S3D Extension*/
				...
				struct videobuf_buffer *buf)
{
	...
	dev->wb_avoid_kick = true; // jonathan.kim@lge.com INDIA camp patch
...
static int activate_resources(struct s3d_ovl_device *dev)
{
	...
	if (dev->fter_config.use_wb) {
		...
		dev->streaming = true;
		dev->wb_start_kicking = false; // jonathan.kim@lge.com INDIA camp patch
		return wb_kick(dev);
	}
...
static void buffer_queue(struct videobuf_queue *q, struct videobuf_buffer *vb)
{
	...
	if (!dev->streaming) {
		S3DINFO("WB 1queue:%d\n", vb->i); // jonathan.kim@lge.com INDIA camp patch
...
static int vidioc_streamon(struct file *file, void *fh, enum v4l2_buf_type i)
{
	...
	* resource allocation and configuration is made*/
	...
	//set_default_window(dev, &dev->win); // jonathan.kim@lge.com fix s3d overlay destination window issue on (camera) preview.
