...
int check_fb_var(struct fb_info *fbi, struct fb_var_screeninfo *var)
{
	...
	//LGE_CHANGE [taekeun1.kim@lge.com] 2011-03-02, P920 : For WBT
	if(display == NULL) {
		return -EINVAL;
	}

	//LGE_CHANGE [taekeun1.kim@lge.com] 2011-03-09, P920 : For WBT
...
static int omapfb_blank(int blank, struct fb_info *fbi)
{
	...
	case FB_BLANK_POWERDOWN:
		if (display->state != OMAP_DSS_DISPLAY_ACTIVE)
			goto exit;
/* block temp code
		// LGE_CHANGE_S [Darren.Kang] fill black for preventing trash image during wake-up
