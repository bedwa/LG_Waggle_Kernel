...
int xmd_hsi_power_off(void); // release interrupt and set all the pins to safe

void xmd_hsi_power_on_reset(void); //configure gpios and request irq

//void xmd_hsi_board_init(struct xmd_hsi_platform_data *d); // runtime gpio configuration like keeping wake line to high,
...
void *xmd_ch_read(int chno, int* len); //returns the buffer pointer in which recieved data is stored. should be called from notify_cb. memory will be freed after notify_cb returns.

int xmd_ch_write(int  chno, void *data, int len); //data is copied in local buffer and then its transmitted. Freeing of *data is not done.
