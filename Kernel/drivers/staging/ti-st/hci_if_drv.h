...
/* Debug macros*/
#define HCIIFDRV_ERR(fmt, arg...)  printk(KERN_ERR "(tibtdrv):"fmt"\n" , ## arg)
#if defined(DEBUG)		/* limited debug messages */
#define HCIIFDRV_DBG(fmt, arg...)  printk(KERN_INFO "(tibtdrv):"fmt"\n" , ## arg)
#define HCIIFDRV_VER(fmt, arg...)
#elif defined(VERBOSE)		/* very verbose */
#define HCIIFDRV_DBG(fmt, arg...)  printk(KERN_INFO "(tibtdrv):"fmt"\n" , ## arg)
#define HCIIFDRV_VER(fmt, arg...)  printk(KERN_INFO "(tibtdrv):"fmt"\n" , ## arg)
...
struct hciif_hciCore_t {
	bool						is_hci_core;			/* Is client using HCI CORE */

	long (*hci_core_recv) (struct sk_buff *);		/* If  is_hci_core == TRUE --> Points to the receive function*/
};
...
struct hciif_client {
	...
	spinlock_t				rx_lock;				/* spin lock data for safe-gaurding
												   operations on SKB */
	...
	struct hciif_hciCore_t	hci_core;			/* HCI Core related params */
...
struct hciif_handle {
	...
	struct completion			hciif_reg_completed;				/* comepletion handler to synchronize
																   hciif_chrdev_open */

	struct completion			hciif_oper_completed;				/* comepletion handler to synchronize
															    	    Open/Close operation between clients */
	
	char						streg_cbdata;						/* ST registration callback  status */

	char						stoper_cbdata;						/* ST Open/Close operation callback status */																

	unsigned long				state;								/* used locally,to maintain 
																   various BT driver status */
	...
	spinlock_t				hciif_lock;							/* spin lock data for safe-gaurding
																    operations on WriteClientsList */
};
