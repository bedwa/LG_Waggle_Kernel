...
//LGE_CHANGE_S [kibum.lee@lge.com] 2010-12-22, common : for pm debug
...
//LGE_CHANGE_E [kibum.lee@lge.com] 2010-12-22, common : for pm debug
...
//LGE_CHANGE_S [kibum.lee@lge.com] 2011-02-15, common : for wakelock debug
...
//LGE_CHANGE_E [kibum.lee@lge.com] 2011-02-15, common : for wakelock debug



//roy.park@lge.com 2010.11.6 Start-->[
...
int kill_abnormal_active_locks(int type)
{
	...
	list_for_each_entry(abnormal_lock, &active_wake_locks[type], link) {
		if (abnormal_lock->flags & WAKE_LOCK_AUTO_EXPIRE) {
			...
			if(strcmp(abnormal_lock->name, "main") == 0)	//if(abnormal_lock->name =="main") // kibum.lee@lge.com modify
			{
				long timeout = abnormal_lock->expires - jiffies;
