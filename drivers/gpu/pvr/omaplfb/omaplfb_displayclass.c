                            ...
                            IMG_VOID   *pvData)
{
	...
	{
#endif
// LGE_CHANGE_S [taekeun1.kim@lge.com] 2011-01-29, P920: barrier enable
...
#if 0
		/* LGE_CHANGE_S [wonki.choi@lge.com] 2010-10-13 S3D Extension*/
		extern int barrier_init_hotkey(bool enable);

//		printk("hPriavate Tag222 0x%x\n", psFlipCmd->hPrivateTag);
		if ( psFlipCmd->hPrivateTag==0xBA551E5 )
			barrier_init_hotkey(1);
		else
			barrier_init_hotkey(0);
#endif
#endif
		OMAPLFBFlip(psSwapChain,
...
#endif
#endif
			/* LGE_CHANGE_E [wonki.choi@lge.com] 2010-10-13 */
			OMAPLFBFlip(psSwapChain,
				(unsigned long)psBuffer->sSysAddr.uiAddr);
			psFlipItem->bFlipped = OMAP_TRUE;
		}
...
/* LGE_CHANGE : [p920] DPI calculation taekeun1.kim@lge.com */
