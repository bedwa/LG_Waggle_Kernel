...
static void fm_core_tx_tasklet(unsigned long arg)
{
	...
	if (atomic_read(&fmdev->tx_cnt)) {
      ...
      // TI_SOLDEL 2011/03.14 cmlee, suppress WBT error [START]
	// Don't worry. just for logging.
      int temp = (int)*(&skb);
      // TI_SOLDEL 2011/03.14 cmlee, suppress WBT error [END]

			ret = fm_st_send(skb);
			if (ret < 0) {
				fmdev->response_completion = NULL;
        // TI_SOLDEL 2011/03.14 cmlee, suppress WBT error [START]
        //FM_DRV_ERR ("TX tasklet failed to send skb(%p)", skb);
