...
//LGE_CHANGE_S [kibum.lee@lge.com] 2011-02-28, common : wakelock time optimization // 5 -> 1 -> 2
//	if (omap_is_console_port(&up->port))
//		wake_lock_timeout(&uart_lock, 5 * HZ);
	if (omap_is_console_port(&up->port))
		wake_lock_timeout(&uart_lock, 2.5 * HZ);
//LGE_CHANGE_E [kibum.lee@lge.com] 2011-02-28, common : wakelock time optimization
...
//LGE_CHANGE_S [kibum.lee@lge.com] 2011-02-28, common : wakelock time optimization 5-> 2
//	if (omap_is_console_port(&up->port))
//		wake_lock_timeout(&uart_lock, 5 * HZ);
	if (omap_is_console_port(&up->port))
		wake_lock_timeout(&uart_lock, 2.5 * HZ);
